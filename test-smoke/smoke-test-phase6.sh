...
skip_phase_if_remote "requires direct port 8087 access and local \$MOBILEBACKUP polling" "PHASE 6"
...
test_start "6.1 full upload — HTTP 200 via Netty"
...
test_start "6.2 full upload — ProcessorService delivered file"
...
test_start "6.3 full upload — content integrity verified"
if $FULL_MOVED; then
...
test_start "6.4 chunked upload — all 3 chunks HTTP 200"
CHUNKED_OK=true
...
test_start "6.5 chunked upload — reassembled and delivered"
...
test_start "6.6 chunked upload — content integrity verified"
if $CHUNKED_MOVED; then
...
if ! $MOBILEBACKUP_SCANNABLE; then
    REASON="mobilebackup dir not in scan1.txt — FileScanner won't index uploads. Add $MOBILEBACKUP to scandir= in $SCAN_CONFIG_DIR/scan1.txt"
    skip "6.7 full upload — getfile.fn download" "$REASON"
    skip "6.8 chunked upload — getfile.fn download" "$REASON"
    skip "6.9 full upload — chunked download" "$REASON"
    skip "6.10 chunked upload — chunked download" "$REASON"
else
...
test_start "6.7 full upload — getfile.fn download verified"
FULL_MD5=""
if $FULL_MOVED; then
...
test_start "6.8 chunked upload — getfile.fn download verified"
CHUNKED_MD5=""
if $CHUNKED_MOVED; then
...
# Helper: compute MD5 of a string/bytes via openssl (portable across macOS/Linux).
# Output: lowercase hex, no "(stdin)=" prefix.
compute_md5_hex() {
    printf '%s' "$1" | openssl md5 2>/dev/null | sed 's/^.*= *//'
}

# Helper: fetch one chunk + its X-Chunk-MD5 response header in a single curl.
# Returns stdout: body then a separator line "---XCMD5---" then the header value.
# Use awk to split after the fact. This avoids two round-trips per chunk.
fetch_chunk_with_md5() {
    local URL="$1"
    curl -s -D - -H "Cookie: uuid=$UUID" "$URL" 2>/dev/null \
        | awk 'BEGIN{h=1} /^\r?$/ && h {h=0; next} h {hdrs=hdrs $0 "\n"; next} {body=body $0} END{print body; print "---XCMD5---"; n=split(hdrs, lines, "\n"); for (i=1;i<=n;i++) { if (tolower(lines[i]) ~ /^x-chunk-md5:/) { sub(/^[^:]*:[ \t]*/, "", lines[i]); gsub(/\r/, "", lines[i]); print lines[i]; exit } } print ""}'
}
...
test_start "6.9 full upload — chunked download (2 chunks) content ver"
...
if $FULL_DL_OK && [ -n "$FULL_MD5" ]; then
    ...
    # Helper: fetch a chunk + its X-Chunk-MD5 header in ONE request.
    # Writes headers to a temp file, body to stdout. Caller reads both.
    set +e
    HDR_TMP_1=$(mktemp)
    CHUNK1_DATA=$(curl -s -D "$HDR_TMP_1" -H "Cookie: uuid=$UUID" \
        "$SERVER/cass/getfile.fn?sNamer=$FULL_MD5&filechunk_size=$CHUNK1_SIZE&filechunk_offset=0")
    CHUNK1_SERVER_MD5=$(tr -d '\r' < "$HDR_TMP_1" | grep -i '^X-Chunk-MD5:' | head -1 | sed 's/^[^:]*:[ \t]*//' | tr -d '[:space:]')
    rm -f "$HDR_TMP_1"

    HDR_TMP_2=$(mktemp)
    CHUNK2_DATA=$(curl -s -D "$HDR_TMP_2" -H "Cookie: uuid=$UUID" \
        "$SERVER/cass/getfile.fn?sNamer=$FULL_MD5&filechunk_size=$CHUNK2_SIZE&filechunk_offset=$CHUNK1_SIZE")
    CHUNK2_SERVER_MD5=$(tr -d '\r' < "$HDR_TMP_2" | grep -i '^X-Chunk-MD5:' | head -1 | sed 's/^[^:]*:[ \t]*//' | tr -d '[:space:]')
    rm -f "$HDR_TMP_2"
    set -e

    # Client-side MD5 of each chunk body
    CHUNK1_CLIENT_MD5=$(compute_md5_hex "$CHUNK1_DATA")
    CHUNK2_CLIENT_MD5=$(compute_md5_hex "$CHUNK2_DATA")

    # MD5 match check (empty server MD5 would count as non-match, surfacing missing header)
    if [ -n "$CHUNK1_SERVER_MD5" ] && [ "$CHUNK1_SERVER_MD5" = "$CHUNK1_CLIENT_MD5" ]; then
        MD5_MATCH_CHUNK1=true
    else
        MD5_MATCH_CHUNK1=false
    fi
    if [ -n "$CHUNK2_SERVER_MD5" ] && [ "$CHUNK2_SERVER_MD5" = "$CHUNK2_CLIENT_MD5" ]; then
        MD5_MATCH_CHUNK2=true
    else
        MD5_MATCH_CHUNK2=false
    fi
...
if $FULL_CHUNK_DL_OK; then
    if [ "$MD5_MATCH_CHUNK1" = "true" ] && [ "$MD5_MATCH_CHUNK2" = "true" ]; then
        pass "6.9 full upload — chunked download (2 chunks) content + X-Chunk-MD5 verified"
        printf "${DIM}       chunk1 MD5: %s (%d bytes)  chunk2 MD5: %s (%d bytes)${RESET}\n" \
            "$CHUNK1_SERVER_MD5" "$CHUNK1_SIZE" "$CHUNK2_SERVER_MD5" "$CHUNK2_SIZE"
    else
        fail "6.9 full upload — chunked download" "content OK but X-Chunk-MD5 mismatch (chunk1:$MD5_MATCH_CHUNK1 chunk2:$MD5_MATCH_CHUNK2 — server sent '$CHUNK1_SERVER_MD5'/'$CHUNK2_SERVER_MD5', client computed '$CHUNK1_CLIENT_MD5'/'$CHUNK2_CLIENT_MD5')"
    fi
else
    if ! $FULL_DL_OK; then
...
if $CHUNKED_DL_OK && [ -n "$CHUNKED_MD5" ]; then
    ...
    # Fetch each chunk + its X-Chunk-MD5 header in ONE request per chunk.
    # Using `curl -D headerfile` avoids issues where the server may not handle
    # HEAD requests properly on getfile.fn.
    set +e
    HDR_C1=$(mktemp)
    C1_DATA=$(curl -s -D "$HDR_C1" -H "Cookie: uuid=$UUID" \
        "$SERVER/cass/getfile.fn?sNamer=$CHUNKED_MD5&filechunk_size=$C1_SIZE&filechunk_offset=0")
    C1_SERVER_MD5=$(tr -d '\r' < "$HDR_C1" | grep -i '^X-Chunk-MD5:' | head -1 | sed 's/^[^:]*:[ \t]*//' | tr -d '[:space:]')
    rm -f "$HDR_C1"

    HDR_C2=$(mktemp)
    C2_DATA=$(curl -s -D "$HDR_C2" -H "Cookie: uuid=$UUID" \
        "$SERVER/cass/getfile.fn?sNamer=$CHUNKED_MD5&filechunk_size=$C2_SIZE&filechunk_offset=$C1_SIZE")
    C2_SERVER_MD5=$(tr -d '\r' < "$HDR_C2" | grep -i '^X-Chunk-MD5:' | head -1 | sed 's/^[^:]*:[ \t]*//' | tr -d '[:space:]')
    rm -f "$HDR_C2"

    HDR_C3=$(mktemp)
    C3_DATA=$(curl -s -D "$HDR_C3" -H "Cookie: uuid=$UUID" \
        "$SERVER/cass/getfile.fn?sNamer=$CHUNKED_MD5&filechunk_size=$C3_SIZE&filechunk_offset=$(( C1_SIZE + C2_SIZE ))")
    C3_SERVER_MD5=$(tr -d '\r' < "$HDR_C3" | grep -i '^X-Chunk-MD5:' | head -1 | sed 's/^[^:]*:[ \t]*//' | tr -d '[:space:]')
    rm -f "$HDR_C3"
    set -e

    C1_CLIENT_MD5=$(compute_md5_hex "$C1_DATA")
    C2_CLIENT_MD5=$(compute_md5_hex "$C2_DATA")
    C3_CLIENT_MD5=$(compute_md5_hex "$C3_DATA")

    if [ -n "$C1_SERVER_MD5" ] && [ "$C1_SERVER_MD5" = "$C1_CLIENT_MD5" ]; then MD5_MATCH_C1=true; else MD5_MATCH_C1=false; fi
    if [ -n "$C2_SERVER_MD5" ] && [ "$C2_SERVER_MD5" = "$C2_CLIENT_MD5" ]; then MD5_MATCH_C2=true; else MD5_MATCH_C2=false; fi
    if [ -n "$C3_SERVER_MD5" ] && [ "$C3_SERVER_MD5" = "$C3_CLIENT_MD5" ]; then MD5_MATCH_C3=true; else MD5_MATCH_C3=false; fi
...
if $CHUNKED_CHUNK_DL_OK; then
    if [ "$MD5_MATCH_C1" = "true" ] && [ "$MD5_MATCH_C2" = "true" ] && [ "$MD5_MATCH_C3" = "true" ]; then
        pass "6.10 chunked upload — chunked download (3 chunks) content + X-Chunk-MD5 verified"
        printf "${DIM}       chunk1 MD5: %s (%d bytes)  chunk2 MD5: %s (%d bytes)  chunk3 MD5: %s (%d bytes)${RESET}\n" \
            "$C1_SERVER_MD5" "$C1_SIZE" "$C2_SERVER_MD5" "$C2_SIZE" "$C3_SERVER_MD5" "$C3_SIZE"
    else
        fail "6.10 chunked upload — chunked download" "content OK but X-Chunk-MD5 mismatch (c1:$MD5_MATCH_C1 c2:$MD5_MATCH_C2 c3:$MD5_MATCH_C3 — server '$C1_SERVER_MD5'/'$C2_SERVER_MD5'/'$C3_SERVER_MD5', client '$C1_CLIENT_MD5'/'$C2_CLIENT_MD5'/'$C3_CLIENT_MD5')"
    fi
else
    if ! $CHUNKED_DL_OK; then
...
test_start "6.11 uiv5-style upload (Netty 8087) — all 3 chunks HTTP 200"
...
test_start "6.12 uiv5-style upload (Netty 8087) — reassembled + conte"
...
test_start "6.13 uiv5-style upload (8081→Netty) — all 3 chunks HTTP 200"
...
test_start "6.14 uiv5-style upload (8081→Netty) — reassembled + conte"
