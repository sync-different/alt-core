angular.module('app.controllers').controller('MultiClusterController', function (Conf,ngProgressLite,$state,$sce, $routeParams,$anchorScroll,$filter,$compile,$location,$rootScope,$scope,$timeout) {
	...
	$scope.initSwiper = function () {
		...
		$timeout(() => {
			...
			$scope.swiperInstance = new Swiper('.swiper-container', {
				...
				spaceBetween: 0,
				effect: 'slide',
				initialSlide: $scope.activeSlide || 0,
				on: {
					init: function () {
	...
	function loadClusterQuery(cluster,asyncIN){
		...
		$("#table"+cluster.cluster).off("scroll");
	...
	$scope.processMsg=function(txt){
		...
		if(tags!=null && tags.length>0){
			var tag=tags[0];
			var tagName = tag.slice(tag.indexOf(":") + 1);
