angular.module('app.controllers').controller('MyFilesController', function (Conf, ngProgressLite, $sce, $routeParams, $uibModal, $anchorScroll, $filter, $compile, $location, $rootScope, $scope, $timeout) {
	...
	$scope.showVideo=function(file){
		...
		if(showVideo){
			...
			closeAudio();
			$scope.commentsHeight=($(window).innerWidth()*0.95-270)*(264/640)+45;
			var sMD5=file.nickname;
			$scope.commentMD5=sMD5;
			$scope.msgs=[];
			$scope.lastComment=0;
			$scope.pullComments();
	...
	$scope.loadTranslate=function(selector,md5){
	    fetch('/cass/gettranslate_json.fn?sMD5='+md5)
          .then(response => {
	...
	$scope.downloadFile = function(r) {
		if (r.file_size > 10 * 1024 * 1024) { //10MB
			$scope.downloadFileByChunks(r);
		} else {
			$scope.downloadFileDirect(r);
		}
	};

	$scope.downloadFileDirect = async function(r) {
		try {
			const url = r.file_path_webapp + '&uuid=' + getCookie('uuid');
			let handle = null;
			let writable = null;
			...
			if (window.showSaveFilePicker) {
				const options = {
					...
					types: [{
						description: 'Files',
						accept: { 'application/octet-stream': ['.' + r.name.split('.').pop()] }
					}]
				};
			...
			console.error('Download cancelled or error:', err);
			$scope.downloadComplete = true;
			$scope.hideDownloadModal();
			$scope.$applyAsync();
		}
	...
	$scope.showDownloadModal = function() {
		...
		$scope.downloadComplete = false;
		$scope.isDownloadCancelled = false;
	};	
	...
	$scope.downloadChunk = function(r, offset, chunkSize) {
		return new Promise((resolve, reject) => {
			const url = r.file_path_webapp + '&uuid=' + getCookie('uuid') +
				'&filechunk_size=' + chunkSize + '&filechunk_offset=' + offset;
			...
			xhr.onload = function () {
				if (xhr.status === 200) {
					...
					reject(new Error('Error downloading. Status code: ' + xhr.status));
				}
			};
	...
	$scope.downloadFileByChunks = async function(r) {
		try {
			const chunkSize = CHUNK_SIZE;
			const totalSize = r.file_size;
			...
			for (let i = 0; i < totalChunks; i++) {
				...
				const chunk = await $scope.downloadChunk(r, offset, chunkSize);
