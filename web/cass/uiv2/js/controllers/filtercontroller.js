angular.module('app.controllers').controller('FilterController', function (Conf,ngProgressLite,$state, $routeParams,$anchorScroll,$filter,$compile,$location,$rootScope,$scope) {
	...
	$('#leftMenu,#leftMenu3').mouseenter(function(){
        try{
    ...
    $('#leftMenu2,#leftMenu4').mouseleave(function(){
       try{
	...
	$scope.selRange=function(range){
		$scope.range=range;
		$scope.loadQuery();
	}
	
	$scope.selMRange=function(range){
		$rootScope.mrange=range;
		$scope.loadMQuery();
	}
	
	$scope.selMType=function(type){
		$rootScope.mftype=type;
		$scope.loadMQuery();
	}

	
	$scope.selTag=function(tag){
		$scope.tag=tag;
		$("#txtSearch").val(tag);
	...
	$(document).unbind("updateFilterTags");
	$(document).bind("updateFilterTags",function(e){
		$scope.loadinTags();
	});
