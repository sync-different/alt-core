 ...
 * Displays logo, navigation menu, search, upload, and user menu
 */
...
// Inline SVG logo
const HivebotLogo = () => (
  <svg width="120" height="22" viewBox="0 0 182 33" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export function TopNav() {
  ...
  const loadSuggestions = useCallback(async (query: string) => {
    ...
    try {
      const results = await fetchSearchSuggestions(query, {
        ftype,
        days: range === '.all' ? 0 : parseInt(range, 10),
      });
  ...
  const handleSearchChange = (_event: React.SyntheticEvent, value: string) => {
    ...
    // If value is cleared (empty string), immediately update the search
    if (value === '') {
  ...
  return (
    ...
    >
      <Toolbar sx={{ gap: 2, position: 'relative', minHeight: 64 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Button
            color="inherit"
            onClick={() => navigate('/home')}
            startIcon={<HomeIcon sx={{ color: '#10B981' }} />}
            sx={{
          ...
          <Button
            color="inherit"
            onClick={() => navigate('/files/.all/.all')}
            startIcon={<FileIcon sx={{ color: '#F59E0B' }} />}
            sx={{
              fontWeight: isActive('/files') ? 'bold' : 'normal',
              borderBottom: isActive('/files') ? '2px solid white' : 'none',
              borderRadius: 0,
              px: 2,
            }}
          >
          ...
          <Button
            ...
            sx={{
              fontWeight: isActive('/folders') ? 'bold' : 'normal',
              borderBottom: isActive('/folders') ? '2px solid white' : 'none',
              borderRadius: 0,
              px: 2,
            }}
          >
          ...
          <Button
            ...
            startIcon={<BackupIcon sx={{ color: '#3B82F6' }} />}
            sx={{
              fontWeight: isActive('/backup') ? 'bold' : 'normal',
              borderBottom: isActive('/backup') ? '2px solid white' : 'none',
              borderRadius: 0,
              px: 2,
            }}
          >
          ...
          <Button
            ...
            startIcon={<ShareIcon sx={{ color: '#EC4899' }} />}
            sx={{
              fontWeight: isActive('/shares') ? 'bold' : 'normal',
              borderBottom: isActive('/shares') ? '2px solid white' : 'none',
              borderRadius: 0,
              px: 2,
            }}
          >
          ...
          <Button
            ...
            startIcon={<CloudIcon sx={{ color: '#8B5CF6' }} />}
            sx={{
              fontWeight: isActive('/clusters') ? 'bold' : 'normal',
              borderBottom: isActive('/clusters') ? '2px solid white' : 'none',
              borderRadius: 0,
              px: 2,
            }}
          >
          ...
          <form onSubmit={handleSearchSubmit} style={{ width: '100%', maxWidth: 500 }}>
            <Autocomplete
              ...
              onChange={(_event, value) => {
                if (value) {
                  setSearchValue(value);
                  // Immediately update Redux store with the selected value
