 ...
 * @author guilespi
 */
public class RelayVaultService implements Runnable {
    ...
    */
    public class ClusterDelegate {
        ...
        private GetMethod httpLocalRequest(String url, String stickyName, String stickyValue) {
        ...
        */
        private void fileRequest(String requestId, String authCookie, String queryString, RelayBridge bridge, String StickyName, String StickyValue) {
            ...
            GetMethod file = httpLocalRequest(url, StickyName, StickyValue);
            try {
                if (file != null) {
        ...
        */
        public void standardRequest(String requestId, 
            ...
            GetMethod response = httpLocalRequest(url, stickyName, stickyValue);
            try {
        ...
        */
        public void loginRequest(String requestId, String user, String password, String encdata, RelayBridge bridge) {
            ...
            GetMethod login = httpLocalRequest(url, null, null);
            try {
                if (login != null) {
                    String responseBody = login.getResponseBodyAsString();
                    String loginResponse = responseBody;
                    Header cookieHeader = login.getResponseHeader("Set-Cookie");
                    if (cookieHeader != null) {
                        HeaderElement[] cookies = cookieHeader.getElements();
                        if (cookies.length > 0) {
    ...
    */
    public class RelayBridge {
        ...
        */
        private int postFileResponse(String requestId, InputStream response, String stickyName, String stickyValue) {
            ...
            if (stickyName != null) {
                ...
                postFile.setRequestHeader("Cookie", stickyName + "=" + stickyValue + ";");                                
            }

            try {
                postFile.setRequestEntity(new InputStreamRequestEntity(response));
        ...
        */
        private int postErrorResponse(String requestId, String response) {
           String responseUrl = String.format("%s://%s:%s/clusters/%s/request-failed/%s", 
                                              _protocol, _relayHost, _relayPort, _clusterId, requestId); 
            PostMethod postMethod = new PostMethod(responseUrl);
            try {
                StringRequestEntity requestEntity = new StringRequestEntity(response, "text", "UTF-8");
        ...
        */
        private int postStringResponse(String requestId, String response, String stickyName, String stickyValue) {
            ...
            if (stickyName != null) {
                ...
                postMethod.setRequestHeader("Cookie", stickyName + "=" + stickyValue + ";");                                
            }
        ...
        */
        private boolean unregisterCluster(String clusterId, String clusterToken) {
            ...
            GetMethod unregister = new GetMethod(url);
            try {
                HttpClient httpclient = new HttpClient();
                int status = httpclient.executeMethod(unregister);
                if (status == 200) {
        ...
        */
        private String registerCluster(String clusterId, String clusterName) {
            ...
            try {
                ...
                StringRequestEntity requestEntity = new StringRequestEntity(data.toJSONString(), "text", "UTF-8");
                postMethod.setRequestEntity(requestEntity);
                HttpClient httpclient = new HttpClient();
                ...
                switch (status) {
                    ...
                    case 200:
                        ...
                        String token = postMethod.getResponseBodyAsString();
        ...
        */
        public void processRequests() throws InterruptedException {
            ...
            while(!_stopped) {
                GetMethod pendingRequests = new GetMethod(url);
                try {
                    HttpClient httpclient = new HttpClient();
                    log("Executing process request url: " + url, 2);
                    int status = httpclient.executeMethod(pendingRequests);
                    final String response = pendingRequests.getResponseBodyAsString();
                    switch (status) {
                        case 200:
                    ...
                    pendingRequests.releaseConnection();
                }
            }
    ...
    /*
        ...
        eaten by Java.
    */
    public RelayVaultService(String bridgeHost, String bridgePort, boolean secure, String clusterHost,
